...
var channelsPath = path.join(__dirname, '..', '..', 'dashboard', 'static', 'js', 'channels.js');
var channelsSource = fs.readFileSync(channelsPath, 'utf8');
var lxmfPath = path.join(__dirname, '..', '..', 'dashboard', 'static', 'js', 'lxmf.js');
var lxmfSource = fs.readFileSync(lxmfPath, 'utf8');
var uiSharedPath = path.join(__dirname, '..', '..', 'dashboard', 'static', 'js', 'ui_shared.js');
...
assert(channelsSource.includes('RS.composer.shouldSendOnEnter(event)'));
assert(lxmfSource.includes('RS.composer.shouldSendOnEnter(e)'));
// A native tablet wider than the legacy 1024px touch threshold still has a
// mobile Return key. It must never submit either composer by accident.
policyContext.isMobile = function() { return false; };
mobileTypingPlatform = true;
assert.strictEqual(policyContext.RS.composer.shouldSendOnEnter({ key: 'Enter' }), false);
mobileTypingPlatform = false;
assert.strictEqual(policyContext.RS.composer.shouldSendOnEnter({ key: 'Enter' }), true);
assert.strictEqual(policyContext.RS.composer.shouldSendOnEnter({ key: 'Enter', shiftKey: true }), false);
assert.strictEqual(policyContext.RS.composer.shouldSendOnEnter({ key: 'Enter', isComposing: true }), false);
policyContext.isMobile = function() { return true; };
assert.strictEqual(policyContext.RS.composer.shouldSendOnEnter({ key: 'Enter' }), false);
