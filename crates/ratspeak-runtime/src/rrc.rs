...
/// Largest millisecond timestamp ECMAScript `Date` can represent.
pub const MAX_DISPLAY_TIMESTAMP_MS: u64 = 8_640_000_000_000_000;
...
/// Keep wire-compatible unsigned timestamps while protecting persistence and
/// UI consumers from values outside ECMAScript's displayable date range.
pub fn sanitize_display_timestamp_ms(timestamp_ms: u64, received_at_ms: u64) -> u64 {
    if timestamp_ms <= MAX_DISPLAY_TIMESTAMP_MS {
        timestamp_ms
    } else {
        received_at_ms.min(MAX_DISPLAY_TIMESTAMP_MS)
    }
}
...
mod tests {
    ...
    #[test]
    fn display_timestamp_sanitizer_preserves_wire_values_until_the_ui_boundary() {
        assert_eq!(
            sanitize_display_timestamp_ms(MAX_DISPLAY_TIMESTAMP_MS, 123),
            MAX_DISPLAY_TIMESTAMP_MS
        );
        assert_eq!(
            sanitize_display_timestamp_ms(MAX_DISPLAY_TIMESTAMP_MS + 1, 456),
            456
        );
        assert_eq!(
            sanitize_display_timestamp_ms(u64::MAX, u64::MAX),
            MAX_DISPLAY_TIMESTAMP_MS
        );
    }
